...
def test_ub_geometric_data(
    ...
    """Test UB profile geometric data matches JSON data."""
...
def test_ub_massprop_data(
    ...
    """Test UB profile mass property matches JSON data."""
    # Delete not used variables to avoid linting errors
    del (
        h_mm,
        b_mm,
...
def test_ub_polygon(
    ...
    poly2 = UB.get_polygon(profile_name)
    assert_geometries_equal(poly1, poly2)


@pytest.mark.parametrize('invalid_name', ['W 100x100', 'W40x490'])
def test_ub_invalid_name(
    invalid_name,
):
    """Test UB profile with invalid name raises ValueError."""
    # Invalid name for constructor
    with pytest.raises(ValueError):
        UB(invalid_name)
    # Invalid name for class method
    with pytest.raises(ValueError):
        UB.get_polygon(invalid_name)
...
def test_ub_massprops_polygon(
    ...
    """Test UB profile massprops from polygon."""
    # Delete not used variables to avoid linting errors
    del (
        tw_mm,
        tf_mm,
    ...
    # Create geometry
    geo = SurfaceGeometry(UB.get_polygon(profile_name), material=steel)
    # Create the section
