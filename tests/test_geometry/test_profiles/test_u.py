...
@pytest.mark.parametrize(
    (
        ...
        'tw_mm, tf_mm, r1_mm, r2_mm, '
        'A_cm2, Iy_cm4, Iz_cm4, '
        'Wely_cm3, Welz_cm3'
    ),
    load_U_profiles_data(),
)
def test_U_geometric_data(
    profile_name,
    h_mm,
    b_mm,
    tw_mm,
    ...
    """Test U profile geometric data matches JSON data."""
...
def test_U_massprop_data(
    ...
    """Test U profile mass property matches JSON data."""
    # Delete not used variables to avoid linting errors
    del (
        h_mm,
        b_mm,
    ...
    assert math.isclose(profile.Iy, Iy_cm4 * 1e4, rel_tol=5e-3)
    assert math.isclose(profile.Iz, Iz_cm4 * 1e4, rel_tol=1e-2)
    assert math.isclose(profile.Wely, Wely_cm3 * 1e3, rel_tol=5e-3)
...
def test_U_polygon(
    ...
    poly2 = U.get_polygon(profile_name)
    assert_geometries_equal(poly1, poly2)


@pytest.mark.parametrize('invalid_name', ['U1000', 'U318', '224', 1400.0, -10])
def test_U_invalid_name(
    invalid_name,
):
    """Test U profile with invalid name raises ValueError."""
    # Invalid name for constructor
    with pytest.raises(ValueError):
        U(invalid_name)
    # Invalid name for class method
    with pytest.raises(ValueError):
        U.get_polygon(invalid_name)
...
def test_U_massprops_polygon(
    ...
    """Test U profile massprops from polygon."""
    # Delete not used variables to avoid linting errors
    del (tw_mm, tf_mm, r1_mm, r2_mm, b_mm, Welz_cm3)
    ...
    # Create geometry
    geo = SurfaceGeometry(U.get_polygon(profile_name), material=steel)
    # Create the section
    ...
    assert math.isclose(Iy_cm4 * 1e4, iy_section, rel_tol=5e-3)
    assert math.isclose(Iz_cm4 * 1e4, iz_section, rel_tol=1e-2)
    assert math.isclose(Wely_cm3 * 1e3, wy_section, rel_tol=5e-3)
