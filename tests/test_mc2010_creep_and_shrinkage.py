...
@pytest.mark.parametrize(
    'sigma, fcm, expected',
    [(5, 20, None), (10, 20, None), (15, 20, None)],  # Raises ValueError
)
def test_check_initial_stress(sigma, fcm, expected):
    """Test check_initial_stress function."""
    try:
        assert (
            _concrete_creep_and_shrinkage._check_initial_stress(sigma, fcm)
            == expected
        )
    except ValueError:
        with pytest.raises(ValueError) as exc_info:
            _concrete_creep_and_shrinkage._check_initial_stress(
                sigma, fcm
            ) == expected
...
def test_check_cem_strength_class(cem_class, expected):
    ...
    try:
        assert (
            _concrete_creep_and_shrinkage._check_cem_strength_class(cem_class)
            == expected
        )
    except ValueError:
        with pytest.raises(ValueError) as exc_info:
            _concrete_creep_and_shrinkage._check_cem_strength_class(
                cem_class
            ) == expected
...
@pytest.mark.parametrize(
    ...
    [
        (
            "32.5 R",
            {"alpha": 0, "alpha_as": 700, "alpha_ds1": 4, "alpha_ds2": 0.012},
        ),
        (
            "42.5 N",
            {"alpha": 0, "alpha_as": 700, "alpha_ds1": 4, "alpha_ds2": 0.012},
        ...
        (
            "42.5 R",
            {"alpha": 1, "alpha_as": 600, "alpha_ds1": 6, "alpha_ds2": 0.012},
        ),
        (
            "52.5 N",
            {"alpha": 1, "alpha_as": 600, "alpha_ds1": 6, "alpha_ds2": 0.012},
        ...
        (
            "32.5 N",
            {"alpha": -1, "alpha_as": 800, "alpha_ds1": 3, "alpha_ds2": 0.013},
        ),
    ],
)
def test_get_creep_shrinkage_coeffs(cem_class, expected):
    ...
    assert (
        _concrete_creep_and_shrinkage._get_creep_shrinkage_coeffs(cem_class)
        == expected
    )
...
def test_calc_temp_corr_age(t0, T_cur, expected):
    ...
    assert np.isclose(
        _concrete_creep_and_shrinkage._calc_temp_corr_age(t0, T_cur),
        expected,
        rtol=1e-5,
    )
...
def test_calc_beta_ds(time, ts, notional_size, expected):
    ...
    assert np.isclose(
        _concrete_creep_and_shrinkage._calc_beta_ds(time, ts, notional_size),
        expected,
        rtol=1e-5,
        atol=1e-10,
    )
...
def test_calc_beta_s1(fcm, expected):
    ...
    assert np.isclose(
        _concrete_creep_and_shrinkage._calc_beta_s1(fcm),
        expected,
        rtol=1e-5,
        atol=1e-10,
    )
...
@pytest.mark.parametrize(
    'time, expected',
    [(1, 0.181269), (10, 0.468714), (100, 0.864665), (1000, 0.998208)],
)
def test_calc_beta_au(time, expected):
    """Test _calc_beta_au function."""
    assert np.isclose(
        _concrete_creep_and_shrinkage._calc_beta_au(time), expected, rtol=1e-5
    )
...
def test_calc_alpha_fcm(fcm, expected):
    ...
    assert np.isclose(
        _concrete_creep_and_shrinkage._calc_alpha_fcm(fcm), expected, rtol=1e-5
    )
...
def test_calc_beta_h(notional_size, alpha_fcm, expected):
    ...
    assert np.isclose(
        _concrete_creep_and_shrinkage._calc_beta_h(notional_size, alpha_fcm),
        expected,
        rtol=1e-5,
    )
