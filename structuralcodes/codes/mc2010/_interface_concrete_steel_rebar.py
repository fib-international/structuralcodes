...
class BondStressSlip:
    ...
    def tau_bu_split(
        ...
        if self._tau_bu_split is None:  # Calculate only if not already calculated
            ...
            self._tau_bu_split = (
                    ...
                    * (25 / phi) ** 0.2
                    * (
                            (c_min / phi) ** 0.25  # <-- Differs from MC2010, follows fib Bulletin 72, Appendix A.
                            * (c_max / c_min) ** 0.1
                            + k_m * K_tr
                    )
            )
    ...
    def s_tau_bu_split(
        ...
        if self._s_tau_bu_split is None:  # Calculate only if not already calculated
            ...
            self._s_tau_bu_split = s1_PO * (tau_bu_split / tau_bmax) ** (1 / 0.4)
    ...
    def tau_yield(
        ...
        Returns:
            Bond stress at reinforcement yield.
        """
        if (l_b is None and phi is not None) or (l_b is not None and phi is None):
            raise ValueError("Both l_b and phi must be provided together or both must be excluded.")
