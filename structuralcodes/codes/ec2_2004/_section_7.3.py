...
def xi_1(xi: float, phi_p: float, phi_s: float) -> float:
...
def rho_p_eff(As: float, xi1: float, Ap: float, Ac_eff: float) -> float:
    ...
    Args:
        ...
        xi1 (float): the adjusted ratio of bond according
            to expression (7.5)
...
def wk(sr_max: float, _esm_ecm: float) -> float:
    ...
    Raises:
        ValueError: if any of sr_max or _esm_ecm is less than zero.
    """
    if sr_max < 0:
